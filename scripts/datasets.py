...
from scripts.abdaugs import SnowAugmentation,RainAugmentation,FogAugmentation,CloudsAugmentation
...
class ImageDataset(Dataset):
    def __init__(self, root, transforms_=None, unaligned=False, mode='test', is_validation=False, Inference=False):
        ...
        self.SnowAugmentation = SnowAugmentation(probability=0.2, flake_size=(0.2, 0.7), speed=(0.007, 0.03))
        self.RainAugmentation = RainAugmentation(probability=0.2, intensity_range=(0.1, 0.3))
        self.FogAugmentation = FogAugmentation(probability=0.3)
        self.CloudsAugmentation = CloudsAugmentation(probability=0.15)
        self.mode=mode
